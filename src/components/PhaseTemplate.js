...
export default function PhaseTemplate({
  logo,
  stats,
  charts,
  isAllComplete,
  ...
  return (
    <div ref={templateRef} className={`bg-white p-8 rounded-xl shadow-xl border ${rtl ? 'rtl' : ''} max-w-3xl mx-auto`} dir={rtl ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">{logo}<span className="text-xl font-bold">{lang === 'ar' ? 'قالب ملخص المرحلة' : 'Phase Summary Template'}</span></div>
        {isAllComplete && (
