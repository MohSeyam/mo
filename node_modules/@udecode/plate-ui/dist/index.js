...
var DEFAULT_TAILWIND_CSS = "src/styles/globals.css";
...
// src/utils/registry/index.ts
var baseUrl = process.env.COMPONENTS_REGISTRY_URL ?? "https://platejs.org";
...
async function resolveTree(index, names) {
  ...
  return tree.filter(
    (component, i, self) => self.findIndex((c) => c.name === component.name) === i
  );
}
...
async function createTempSourceFile(filename) {
  const dir = await fs.mkdtemp(path3.join(tmpdir(), "plate-"));
    ...
    for (const item of payload) {
      ...
      if (existingComponent.length > 0 && !options.overwrite) {
        if (selectedComponents.includes(item.name)) {
          ...
          if (!overwrite) {
            logger.info(
              `Skipped ${item.name}. To overwrite, run with the ${chalk2.green(
                "--overwrite"
              )} flag.`
            );
            continue;
...
async function main() {
  ...
  const program = new Command4().name("plate-ui").description("add components and dependencies to your project").version(
