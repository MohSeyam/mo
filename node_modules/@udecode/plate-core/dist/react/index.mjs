...
import { ElementApi as ElementApi2 } from "@udecode/slate";
...
import { ElementApi as ElementApi3 } from "@udecode/slate";
...
import { TextApi as TextApi2 } from "@udecode/slate";
...
var getEdgeNodes = (editor) => {
  ...
  if (!edge) return null;
  const parent = NodeApi2.parent(editor, cursor.path) ?? null;
  const isAffinityInlineElement = (() => {
    if (!parent || !ElementApi4.isElement(parent)) return false;
  ...
  const nodeEntry = isAffinityInlineElement ? [parent, PathApi3.parent(cursor.path)] : [NodeApi2.get(editor, cursor.path), cursor.path];
  ...
  const siblingNode = NodeApi2.get(editor, siblingPath);
...
import { IS_FIREFOX, NodeApi as NodeApi3 } from "@udecode/slate";
import isEqual from "lodash/isEqual.js";
var getMarkBoundaryAffinity = (editor, markBoundary) => {
  const { marks, selection } = editor;
  if (!selection) return;
  const marksMatchLeaf = (leaf) => {
    return marks && isEqual(NodeApi3.extractProps(leaf), marks) && Object.keys(marks).length > 1;
  };
...
import { ElementApi as ElementApi5, NodeApi as NodeApi4 } from "@udecode/slate";
var isNodeAffinity = (editor, node, affinity) => {
  const marks = Object.keys(NodeApi4.extractProps(node));
  const keys = ElementApi5.isElement(node) ? [node.type] : marks;
  return keys.some(
...
import { ElementApi as ElementApi6, NodeApi as NodeApi5 } from "@udecode/slate";
var setAffinitySelection = (editor, edgeNodes, affinity) => {
  ...
  if (affinity === "backward") {
    ...
    if (ElementApi6.isElement(before[0])) return;
    setMarks(null);
    return;
  }
  if (before === null) {
  ...
  select(beforeEnd);
  if (ElementApi6.isElement(after[0])) {
    return;
  } else {
    setMarks(NodeApi5.extractProps(after[0]));
  }
};
  ...
  transforms: {
    ...
    deleteBackward: (unit) => {
      const apply = () => {
        if (unit === "character" && editor.api.isCollapsed()) {
          ...
          const startText = start && (TextApi3.isText(start[0]) ? start[0].text : NodeApi6.string(start[0]));
          deleteBackward(unit);
    ...
    insertText(text, options) {
      const applyOutwardAffinity = () => {
        ...
        const textNode = NodeApi6.get(editor, textPath);
        if (!textNode) {
          return;
        }
        const marks = Object.keys(NodeApi6.extractProps(textNode));
        ...
        if (nextPoint) {
          ...
          nextTextNode = NodeApi6.get(editor, nextTextPath) || null;
        }
...
var hasElement = (edgeNodes) => {
  const [before, after] = edgeNodes;
  return before && ElementApi7.isElement(before[0]) || after && ElementApi7.isElement(after[0]);
};
...
import { ElementApi as ElementApi8, TextApi as TextApi4 } from "@udecode/slate";
...
var htmlElementToLeaf = (editor, element) => {
  ...
  return deserializeHtmlNodeChildren(editor, element).reduce(
    (arr, child) => {
      if (!child) return arr;
      if (ElementApi8.isElement(child)) {
        if (Object.keys(node).length > 0) {
          mergeDeepToNodes({
            node: child,
            query: {
              filter: ([n]) => TextApi4.isText(n)
            },
            source: node
          });
        }
        ...
        if (TextApi4.isText(child) && child.text) {
  ...
  normalizeInitialValue: ({ editor, getOptions }) => {
    ...
    const addNodeId = (entry) => {
      ...
      if (!node[idKey] && queryNode3([node, path], {
        ...
        filter: (entry2) => {
          const [node2] = entry2;
          if (filterText && !ElementApi9.isElement(node2)) {
            return false;
          }
          if (filterInline && ElementApi9.isElement(node2) && !editor.api.isBlock(node2)) {
            return false;
          }
...
import { NodeApi as NodeApi7 } from "@udecode/slate";
var resetBlock = (editor, { at } = {}) => {
  const entry = editor.api.block({ at });
  if (!entry?.[0]) return;
  const [block, path] = entry;
  editor.tf.withoutNormalizing(() => {
    const { id, type, ...otherProps } = NodeApi7.extractProps(block);
...
var PlateElement = React.forwardRef(function PlateElement2({ as: Tag = "div", children, insetProp, ...props }, ref) {
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, inset && /* @__PURE__ */ React.createElement(NonBreakingSpace, null), /* @__PURE__ */ React.createElement(
    Tag,
    {
    ...
    children,
    inset && /* @__PURE__ */ React.createElement(NonBreakingSpace, null)
  ));
});
...
var NonBreakingSpace = () => /* @__PURE__ */ React.createElement("span", { style: { fontSize: 0 }, contentEditable: false }, String.fromCodePoint(160));
var PlateLeaf = React.forwardRef(({ as: Tag = "span", children, inset: insetProp, ...props }, ref) => {
  const attributes = useNodeAttributes(props, ref);
...
var pluginRenderLeaf = (editor, plugin) => function render(props) {
  ...
  if (leaf[plugin.node.type ?? plugin.key]) {
    ...
    return /* @__PURE__ */ React2.createElement(Leaf, { ...defaultProps, ...ctxProps }, children);
  }
  return children;
};
...
var pluginRenderText = (editor, plugin) => function render(nodeProps) {
  ...
  if (text[plugin.node.type ?? plugin.key]) {
    ...
    return /* @__PURE__ */ React4.createElement(Text2, { ...defaultProps, ...ctxProps }, children);
  }
  return children;
};
...
import { PathApi as PathApi5 } from "@udecode/slate";
var useNodePath = (node) => {
  ...
  return useMemoizedSelector(
    ...
    [editor, node],
    (a, b) => {
      return !!a && !!b && PathApi5.equals(a, b);
    }
  );
};
...
function Effect() {
  const path = usePath();
  if (path && PathApi6.equals(path, [0])) {
    return /* @__PURE__ */ React8.createElement(FirstBlockEffect, null);
  }
  return null;
}
...
function ElementContent({ editor, plugin, ...props }) {
  ...
  const defaultProps = Component ? {} : { as: plugin.render?.as };
  let component = /* @__PURE__ */ React9.createElement(Element2, { ...defaultProps, ...props }, children, /* @__PURE__ */ React9.createElement(BelowRootNodes, { ...defaultProps, ...props }));
...
function BelowRootNodes(props) {
  const editor = useEditorRef();
  const readOnly = useReadOnly();
  return /* @__PURE__ */ React9.createElement(React9.Fragment, null, editor.meta.pluginCache.render.belowRootNodes.map((key) => {
    const plugin = editor.getPlugin({ key });
    if (isEditOnly(readOnly, plugin, "render")) return null;
    const Component = plugin.render.belowRootNodes;
    return /* @__PURE__ */ React9.createElement(Component, { key, ...props });
  }));
}
var pluginRenderElement = (editor, plugin) => function render(props) {
  ...
  if (element.type === plugin.node.type) {
    return /* @__PURE__ */ React9.createElement(
      ...
      /* @__PURE__ */ React9.createElement(ElementContent, { editor, plugin, ...props })
    );
  }
};
...
var pipeRenderElement = (editor, renderElementProp) => {
  return function render(props) {
    ...
    return /* @__PURE__ */ React10.createElement(
      ...
      /* @__PURE__ */ React10.createElement(PlateElement, { ...ctxProps }, props.children, /* @__PURE__ */ React10.createElement(BelowRootNodes, { ...ctxProps }))
    );
  };
};
...
var usePlateControllerStore = (idProp) => {
  const storeAtom = React11.useMemo(
    () => atom2((get) => {
...
var usePlateStore = (id) => {
  ...
  const [localStoreExists] = React12.useState(!!localStore.store);
...
var useIncrementVersion = (key, id) => {
  const previousVersionRef = React12.useRef(1);
  const store = usePlateStore(id);
...
var useElementSelector = (selector, deps, {
  ...
  const selectorAtom = React15.useMemo(
    () => selectAtom2(
...
function EditorHotkeysEffect({
  ...
  const editor = useEditorRef(id);
  return /* @__PURE__ */ React16.createElement(React16.Fragment, null, Object.entries(editor.meta.shortcuts).map(
...
function EditorRefEffect({ id }) {
  ...
  return /* @__PURE__ */ React18.createElement(React18.Fragment, null, editor.meta.pluginCache.useHooks.map((key) => {
...
var PlateContainer = ({
  ...
  const mainContainer = /* @__PURE__ */ React21.createElement("div", { id: editor.meta.uid, ref: containerRef, ...props }, children);
...
function PlateSlate({
  ...
  editor.meta.pluginCache.render.aboveSlate.forEach((key) => {
    ...
    aboveSlate = /* @__PURE__ */ React23.createElement(AboveSlate, null, aboveSlate);
  });
  return aboveSlate;
}
...
var PlateContent = React24.forwardRef(
  ({
    ...
    return /* @__PURE__ */ React24.createElement(PlateSlate, { id }, /* @__PURE__ */ React24.createElement(
      EditorStateEffect,
      {
        id,
