...
var NonBreakingSpace = () => /* @__PURE__ */ import_react.default.createElement("span", { style: { fontSize: 0 }, contentEditable: false }, String.fromCodePoint(160));
...
var pluginRenderElementStatic = (editor, plugin) => function render(nodeProps) {
  if (nodeProps.element.type === plugin.node.type) {
    ...
    let component = /* @__PURE__ */ import_react3.default.createElement(Element2, { ...defaultProps, ...nodeProps }, children, editor.meta.pluginCache.render.belowRootNodes.map((key) => {
...
var ElementStatic = import_react6.default.memo(BaseElementStatic, (prev, next) => {
  return (prev.element === next.element || prev.element._memo !== void 0 && prev.element._memo === next.element._memo) && (0, import_slate10.isElementDecorationsEqual)(prev.decorations, next.decorations);
});
function BaseLeafStatic({
  ...
  const decoratedLeaves = import_slate10.TextApi.decorations(text, decorations);
...
var LeafStatic = import_react6.default.memo(BaseLeafStatic, (prev, next) => {
  return (
    // prev.text === next.text &&
    import_slate10.TextApi.equals(next.text, prev.text) && (0, import_slate10.isTextDecorationsEqual)(next.decorations, prev.decorations)
  );
});
var defaultDecorate = () => [];
function Children({
  ...
  return /* @__PURE__ */ import_react6.default.createElement(import_react6.default.Fragment, null, children.map((child, i) => {
    ...
    if (p) {
      ...
      for (const dec of decorations) {
        const d = import_slate10.RangeApi.intersection(dec, range);
        if (d) {
          ds.push(d);
        }
      }
    }
    return import_slate10.ElementApi.isElement(child) ? /* @__PURE__ */ import_react6.default.createElement(
      ElementStatic,
      {
        key: i,
        decorate,
...
var getEdgeNodes = (editor) => {
  ...
  if (!edge) return null;
  const parent = import_slate11.NodeApi.parent(editor, cursor.path) ?? null;
  const isAffinityInlineElement = (() => {
    if (!parent || !import_slate11.ElementApi.isElement(parent)) return false;
  ...
  const nodeEntry = isAffinityInlineElement ? [parent, import_slate11.PathApi.parent(cursor.path)] : [import_slate11.NodeApi.get(editor, cursor.path), cursor.path];
  ...
  const siblingNode = import_slate11.NodeApi.get(editor, siblingPath);
...
var getMarkBoundaryAffinity = (editor, markBoundary) => {
  ...
  const marksMatchLeaf = (leaf) => {
    return marks && (0, import_isEqual.default)(import_slate13.NodeApi.extractProps(leaf), marks) && Object.keys(marks).length > 1;
  };
  ...
  if (selectionDirection === "backward" && marksDirection === "forward")
    return "forward";
  if (import_slate13.IS_FIREFOX && selectionDirection === "forward" && marksDirection !== "backward")
    return "forward";
  return "backward";
};
...
var isNodeAffinity = (editor, node, affinity) => {
  const marks = Object.keys(import_slate14.NodeApi.extractProps(node));
  const keys = import_slate14.ElementApi.isElement(node) ? [node.type] : marks;
  return keys.some(
...
var setAffinitySelection = (editor, edgeNodes, affinity) => {
  ...
  if (affinity === "backward") {
    ...
    if (import_slate15.ElementApi.isElement(before[0])) return;
    setMarks(null);
    return;
  }
  if (before === null) {
  ...
  select(beforeEnd);
  if (import_slate15.ElementApi.isElement(after[0])) {
    return;
  } else {
    setMarks(import_slate15.NodeApi.extractProps(after[0]));
  }
};
  ...
  transforms: {
    ...
    deleteBackward: (unit) => {
      const apply = () => {
        if (unit === "character" && editor.api.isCollapsed()) {
          ...
          const startText = start && (import_slate16.TextApi.isText(start[0]) ? start[0].text : import_slate16.NodeApi.string(start[0]));
          deleteBackward(unit);
    ...
    insertText(text, options) {
      const applyOutwardAffinity = () => {
        ...
        const textNode = import_slate16.NodeApi.get(editor, textPath);
        if (!textNode) {
          return;
        }
        const marks = Object.keys(import_slate16.NodeApi.extractProps(textNode));
        ...
        if (nextPoint) {
          ...
          nextTextNode = import_slate16.NodeApi.get(editor, nextTextPath) || null;
        }
...
var hasElement = (edgeNodes) => {
  const [before, after] = edgeNodes;
  return before && import_slate16.ElementApi.isElement(before[0]) || after && import_slate16.ElementApi.isElement(after[0]);
};
...
var htmlElementToLeaf = (editor, element) => {
  ...
  return deserializeHtmlNodeChildren(editor, element).reduce(
    (arr, child) => {
      if (!child) return arr;
      if (import_slate17.ElementApi.isElement(child)) {
        if (Object.keys(node).length > 0) {
          mergeDeepToNodes({
            node: child,
            query: {
              filter: ([n]) => import_slate17.TextApi.isText(n)
            },
            source: node
          });
        }
        ...
        if (import_slate17.TextApi.isText(child) && child.text) {
...
var withNodeId = ({
  ...
  const overrideIdIfSet = (node) => {
    ...
    if ((0, import_utils13.isDefined)(node._id)) {
      const id = node._id;
      delete node._id;
    ...
    transforms: {
      apply(operation) {
        ...
        if (operation.type === "split_node") {
          ...
          if ((0, import_slate18.queryNode)([node, operation.path], query)) {
      ...
      insertNodes(_nodes, options) {
        const nodes = (0, import_castArray3.default)(_nodes).filter(
          (node) => !!node
        );
...
var NodeIdPlugin = createTSlatePlugin({
  ...
  normalizeInitialValue: ({ editor, getOptions }) => {
    ...
    const addNodeId = (entry) => {
      ...
      if (!node[idKey] && (0, import_slate19.queryNode)([node, path], {
        allow,
        exclude,
        filter: (entry2) => {
          const [node2] = entry2;
          if (filterText && !import_slate19.ElementApi.isElement(node2)) {
            return false;
          }
          if (filterInline && import_slate19.ElementApi.isElement(node2) && !editor.api.isBlock(node2)) {
            return false;
          }
...
var insertExitBreak = (editor, { match, reverse } = {}) => {
  ...
  const target = editor.api.above({
    at: block[1],
    match: (0, import_slate20.combineMatchOptions)(
      editor,
  ...
  const targetPath = reverse ? ancestorPath : import_slate20.PathApi.next(ancestorPath);
...
var resetBlock = (editor, { at } = {}) => {
  ...
  editor.tf.withoutNormalizing(() => {
    const { id, type, ...otherProps } = import_slate21.NodeApi.extractProps(block);
...
var SlateExtensionPlugin = createSlatePlugin({
   ...
   */
  init: (0, import_utils15.bindFirst)(init, editor),
  insertExitBreak: (0, import_utils15.bindFirst)(insertExitBreak, editor),
  resetBlock: (0, import_utils15.bindFirst)(resetBlock, editor),
  setValue: (0, import_utils15.bindFirst)(setValue, editor)
}));

// src/lib/utils/normalizeDescendantsToDocumentFragment.ts
var isInlineNode = (editor) => (node) => import_slate22.TextApi.isText(node) || import_slate22.ElementApi.isElement(node) && editor.api.isInline(node);
