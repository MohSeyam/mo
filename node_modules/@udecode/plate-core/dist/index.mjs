...
var pluginRenderElementStatic = (editor, plugin) => function render(nodeProps) {
  if (nodeProps.element.type === plugin.node.type) {
    ...
    let component = /* @__PURE__ */ React3.createElement(Element2, { ...defaultProps, ...nodeProps }, children, editor.meta.pluginCache.render.belowRootNodes.map((key) => {
...
var pluginRenderTextStatic = (editor, plugin) => function render(nodeProps) {
  ...
  if (text[plugin.node.type ?? plugin.key]) {
    ...
    const defaultProps = Component ? {} : { as: plugin.render?.as };
    return /* @__PURE__ */ React5.createElement(Text2, { ...defaultProps, ...ctxProps }, children);
  }
  return children;
};
...
function Children({
  ...
  return /* @__PURE__ */ React6.createElement(React6.Fragment, null, children.map((child, i) => {
    ...
    return ElementApi4.isElement(child) ? /* @__PURE__ */ React6.createElement(
      ElementStatic,
      {
        key: i,
        decorate,
...
import {
  ElementApi as ElementApi11,
  TextApi as TextApi6
} from "@udecode/slate";

// src/lib/plugins/affinity/AffinityPlugin.ts
import {
  ElementApi as ElementApi8,
  NodeApi as NodeApi6,
  TextApi as TextApi4
} from "@udecode/slate";
...
var getEdgeNodes = (editor) => {
  ...
  const isAffinityInlineElement = (() => {
    if (!parent || !ElementApi5.isElement(parent)) return false;
...
import { ElementApi as ElementApi6, NodeApi as NodeApi4 } from "@udecode/slate";
var isNodeAffinity = (editor, node, affinity) => {
  const marks = Object.keys(NodeApi4.extractProps(node));
  const keys = ElementApi6.isElement(node) ? [node.type] : marks;
  return keys.some(
...
import { ElementApi as ElementApi7, NodeApi as NodeApi5 } from "@udecode/slate";
var setAffinitySelection = (editor, edgeNodes, affinity) => {
  ...
  if (affinity === "backward") {
    ...
    if (ElementApi7.isElement(before[0])) return;
    setMarks(null);
    return;
  }
  if (before === null) {
  ...
  select(beforeEnd);
  if (ElementApi7.isElement(after[0])) {
    return;
  } else {
    ...
    deleteBackward: (unit) => {
      const apply = () => {
        if (unit === "character" && editor.api.isCollapsed()) {
          ...
          const startText = start && (TextApi4.isText(start[0]) ? start[0].text : NodeApi6.string(start[0]));
          deleteBackward(unit);
...
var hasElement = (edgeNodes) => {
  const [before, after] = edgeNodes;
  return before && ElementApi8.isElement(before[0]) || after && ElementApi8.isElement(after[0]);
};
...
import { ElementApi as ElementApi9, TextApi as TextApi5 } from "@udecode/slate";
...
var htmlElementToLeaf = (editor, element) => {
  ...
  return deserializeHtmlNodeChildren(editor, element).reduce(
    (arr, child) => {
      if (!child) return arr;
      if (ElementApi9.isElement(child)) {
        if (Object.keys(node).length > 0) {
          mergeDeepToNodes({
            node: child,
            query: {
              filter: ([n]) => TextApi5.isText(n)
            },
            source: node
          });
        }
        ...
        if (TextApi5.isText(child) && child.text) {
  ...
  return {
    transforms: {
      ...
      insertNodes(_nodes, options) {
        const nodes = castArray3(_nodes).filter(
          (node) => !!node
        );
  ...
  normalizeInitialValue: ({ editor, getOptions }) => {
    ...
    const addNodeId = (entry) => {
      ...
      if (!node[idKey] && queryNode3([node, path], {
        ...
        filter: (entry2) => {
          const [node2] = entry2;
          if (filterText && !ElementApi10.isElement(node2)) {
            return false;
          }
          if (filterInline && ElementApi10.isElement(node2) && !editor.api.isBlock(node2)) {
            return false;
          }
...
// src/lib/utils/normalizeDescendantsToDocumentFragment.ts
var isInlineNode = (editor) => (node) => TextApi6.isText(node) || ElementApi11.isElement(node) && editor.api.isInline(node);
...
var normalize = (descendants, isInline, makeDefaultBlock) => {
  ...
  descendants = descendants.map((node) => {
    if (ElementApi11.isElement(node)) {
      return {
        ...node,
        children: normalize(
          node.children,
